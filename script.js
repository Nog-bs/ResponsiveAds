...
const type = "grass";
const answer1 = pokes.filter(item => item.types.includes(type)).map(item => item.name);
...
const answer2Or = pokes.filter(item => types.some(type => item.types.includes(type))).map(item => item.name);
const answer2And = pokes.filter(item => types.every(type => item.types.includes(type))).map(item => item.name);
